...
function clearAddTaskForm() {
    let medPrio = document.getElementById("mediumPriority");
    if(medPrio) medPrio.checked = true;
    
    let title = document.getElementById("taskTitle");
    if(title) title.value = "";
    
    let desc = document.getElementById("taskDescription");
    if(desc) desc.value = "";
    
    let dueDate = document.getElementById("taskDueDate");
    if(dueDate) dueDate.value = new Date().toISOString().split('T')[0];
    
    let cat = document.getElementById("categoryInput");
    if(cat) cat.value = "";
